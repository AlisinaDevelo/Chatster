...
## Acceptance criteria
- [x] Initialize an OTel `TracerProvider` only when configured, with `service.name=chatster` and clean shutdown flushing.
- [x] Trace HTTP health/history requests, WebSocket upgrade/connection lifecycle, message persistence, and broadcast enqueue work with useful room-safe attributes.
- [x] Use standard OTel configuration where possible (`OTEL_SERVICE_NAME`, OTLP endpoint/exporter settings); the default local and CI path remains exporter-free.
- [x] Add tests with an in-memory exporter or equivalent that verify spans exist and message bodies/secrets are absent.
- [x] Document collector setup, sampling, shutdown behavior, and the relationship between traces, Prometheus metrics, and `slog` logs.
